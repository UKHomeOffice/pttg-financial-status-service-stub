...
setup() {
	...
	ARTIFACT="pttg-financial-status-service-stub-${VERSION_NUMBER}.jar"

	if [ -n "$BUILD_NUMBER" ]; then
		ARTIFACT="pttg-financial-status-service-stub-${VERSION_NUMBER}.${BUILD_NUMBER}.jar"
	fi
...
cleanup() {
	...
	IMAGES=$(docker images | grep pttg-financial-status-service-stub-build | awk '{print $3'})
...
createDockerImageToExecuteBuildIn() {
	...
	docker build -f src/main/docker/Dockerfile.build -t pttg-financial-status-service-stub-build .
}

executeBuild() {
	echo "running docker image as named container - pttg-financial-status-service-stub-build"
	docker run --name pttg-financial-status-service-stub-build\
		-e "VERSION_NUMBER=${VERSION_NUMBER}" \
		-e "BUILD_NUMBER=${BUILD_NUMBER}" \
		pttg-financial-status-service-stub-build
}
...
buildDockerAppExecutionImage() {
	...
	if [[ -n ${BUILD_NUMBER} ]]; then
		TAG="quay.io/ukhomeofficedigital/pttg-financial-status-service-stub:${VERSION_NUMBER}.${BUILD_NUMBER}"
	else
		TAG="quay.io/ukhomeofficedigital/pttg-financial-status-service-stub:${VERSION_NUMBER}"
	fi
	echo "building " ${TAG}
	docker build -t ${TAG} .
}
